					 ...
					 unsigned long flags)
{
	...
	mpol = mpol_dup(current->mempolicy);
	if (IS_ERR(mpol)) {
		ret = PTR_ERR(mpol);
		return ret;
	}
	if (mpol->mode != MPOL_INTERLEAVE_WEIGHT) {
		stale = mpol;
		ret = -EOPNOTSUPP;
		goto err_out;
	}

	INIT_LIST_HEAD(&mpol->weight_list);
	for (i = 0; i < weight_count; i++) {
		...
		list_add_tail(&new_iw->list, &mpol->weight_list);
	}

	task_lock(current);
	stale = current->mempolicy;
...
static const struct kobj_type iw_kobj_ktype = {
